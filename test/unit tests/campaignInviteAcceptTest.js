...
let {newUser, newCampaign, newThread, campaignInvite, campaignInviteAccept, CHAT} = controls;
...
runTest = async() => {
  ...
  // let campaignThread;
  // [campaign, campaignThread, testUser2, invite] = await Promise.all([
  //   Campaign.findById(campaign._id).exec(),
  //   Thread.findById(campaign.threadId).exec(),
  //   User.findById(testUser2._id).exec(),
  //   Message.findById(invite._id).exec()
  // ])

  // console.log({accept}, {campaign}, {campaignThread}, {testUser2}, {invite});

  deepPrint(await campaignInviteAccept({
    sender: testUser2._id,
    directRecipient: testUser._id,
    origMessage: invite._id,
    campaignId: campaign._id,
