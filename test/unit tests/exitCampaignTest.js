...
let {newUser, newCampaign, newThread, campaignInvite, campaignInviteAccept, CHAT, exitCampaign} = controls;
...
runTest = async() => {
  ...
  // let exit = await exitCampaign({
  //   sender: testUser2._id,
  //   campaignId: campaign._id,
  //   threads: [campaign.threadId],
  //   text: "I'm sorry I just can't do this anymore"
  // });

  // [campaignThread, testUser2] = await Promise.all([
  //   Thread.findById(campaignThread._id).exec(),
  //   User.findById(testUser2._id).exec()
  // ]);

  // console.log({exit}, {campaign}, {campaignThread}, {testUser2});

  deepPrint(await exitCampaign({
    sender: testUser2._id,
    campaignId: campaign._id,
    threads: [campaign.threadId],
    text: "I'm sorry I just can't do this anymore"
