...
let {newUser, newCampaign, newThread, campaignInvite, campaignInviteReject, CHAT} = controls;
...
runTest = async() => {
  ...
  // let reject = await campaignInviteReject({
  //   sender: testUser2._id,
  //   directRecipient: testUser._id,
  //   origMessage: invite._id,
  //   campaignId: campaign._id,
  //   threadIds: [testThread._id, campaign.threadId],
  //   text: "Lol no that's stupid I have video games"
  // })


  // let campaignThread;
  // [campaign, campaignThread, invite] = await Promise.all([
  //   Campaign.findById(campaign._id).exec(),
  //   Thread.findById(campaign.threadId),
  //   Message.findById(invite._id).exec()
  // ])

  // console.log({reject}, {campaign}, {campaignThread}, {invite});


  deepPrint(await campaignInviteReject({
    sender: testUser2._id,
    directRecipient: testUser._id,
    origMessage: invite._id,
    campaignId: campaign._id,
